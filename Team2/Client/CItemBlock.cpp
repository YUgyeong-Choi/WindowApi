...
CItemBlock::CItemBlock():m_bActive(true)
{
}
...
void CItemBlock::Render(HDC hDC)
{
	if (m_bActive) {
		...
		HBRUSH myBrush = (HBRUSH)CreateSolidBrush(RGB(200, 153, 213));
		HBRUSH oldBrush = (HBRUSH)SelectObject(hDC, myBrush);
		Rectangle(hDC, m_tRect, { CScrollManager::GetInstance()->GetScrollX(), 0 });
		SelectObject(hDC, oldBrush);
		DeleteObject(myBrush);
...
void CItemBlock::OnCollision(CObject* _op)
{
